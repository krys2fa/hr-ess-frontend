  ...
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-xl p-4 shadow">
      <div class="grid grid-cols-8 font-bold border-b pb-2 table-header">
        <span>Name</span><span>Staff ID</span><span>Branch</span><span>Department</span><span>Role</span><span>Email</span><span>Status</span><span>Actions</span>
      </div>
      <div v-for="emp in employees" :key="emp.id" class="grid grid-cols-8 border-b py-2 items-center">
        <span>{{ emp.name }}</span>
        ...
        <span>{{ emp.role }}</span>
        <span>{{ emp.email }}</span>
        <span><span :class="getStatusClass(emp.status)">{{ emp.status }}</span></span>
        <span class="flex gap-2">
          <button class="action-btn edit" @click="openEditModal(emp)"><PencilSquareIcon class="w-5 h-5" /> Edit</button>
          <button class="action-btn delete" @click="openDeleteModal(emp)"><TrashIcon class="w-5 h-5" /> Delete</button>
        </span>
      </div>
    </div>
      ...
      <Dialog as="div" class="relative z-50" @close="closeModal">
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
          <DialogPanel class="bg-white rounded-lg p-6 w-full max-w-lg shadow-lg">
            <DialogTitle class="text-lg font-bold mb-4">{{ isEdit ? 'Edit Employee' : 'Add Employee' }}</DialogTitle>
            <form @submit.prevent="isEdit ? updateEmployee() : addEmployee()" class="space-y-4">
              <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input v-model="modalData.staffId" class="w-full border rounded px-3 py-2" required />
                </div>
                <div>
                  <label class="block mb-1">Branch</label>
                  <select v-model="modalData.branch" class="w-full border rounded px-3 py-2" required>
                    <option v-for="b in branches" :key="b" :value="b">{{ b }}</option>
                  </select>
                </div>
                <div>
                  <label class="block mb-1">Department</label>
                  <select v-model="modalData.departmentId" class="w-full border rounded px-3 py-2" required>
                    <option v-for="d in departments" :key="d.id" :value="d.id">{{ d.name }}</option>
                  </select>
                </div>
                <div>
                  <label class="block mb-1">Role</label>
                  <select v-model="modalData.roleId" class="w-full border rounded px-3 py-2" required>
                    <option v-for="r in roles" :key="r.id" :value="r.id">{{ r.title }}</option>
                  </select>
                </div>
                <div>
                  <label class="block mb-1">Email</label>
                  <input v-model="modalData.email" type="email" class="w-full border rounded px-3 py-2" required />
                </div>
                <div>
                  <label class="block mb-1">Phone</label>
                  <input v-model="modalData.phone" class="w-full border rounded px-3 py-2" required />
                </div>
                <div>
      ...
      <Dialog as="div" class="relative z-50" @close="closeDeleteModal">
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
          <DialogPanel class="bg-white rounded-lg p-6 w-full max-w-sm shadow-lg">
            <DialogTitle class="text-lg font-bold mb-4">Delete Employee</DialogTitle>
            <p>Are you sure you want to delete <strong>{{ modalData.name }}</strong> ({{ modalData.staffId }})?</p>
            <div class="flex justify-end gap-2 mt-6">
              <button class="px-4 py-2 bg-gray-200 rounded" @click="closeDeleteModal">Cancel</button>
              <button class="px-4 py-2 bg-red-500 text-white rounded" @click="deleteEmployee">Delete</button>
            </div>
          </DialogPanel>
        </div>
      </Dialog>
...
function openAddModal() {
  isEdit.value = false
  Object.assign(modalData, { id: null, name: '', staffId: '', branch: '', departmentId: null, roleId: null, email: '', phone: '', status: 'Active' })
...
function openEditModal(emp: any) {
  ...
  Object.assign(modalData, {
    ...emp,
    departmentId: departments.value.find(d => d.name === emp.department)?.id ?? null,
...
function addEmployee() {
  employees.value.push({
    ...
    department: departments.value.find(d => d.id === modalData.departmentId)?.name || '',
